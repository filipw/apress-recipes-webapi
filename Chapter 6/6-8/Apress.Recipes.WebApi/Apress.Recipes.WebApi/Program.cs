    ...
    class Program
    {
        static void Main()
        {
            ...
            using (WebApp.Start<Startup>(address))
            {
                ...
                var msgHeader = new HttpRequestMessage(HttpMethod.Get, address + "items/1");
                var resHeader = client.SendAsync(msgHeader).Result;
                Print(resHeader);

                var msgHeaderV2 = new HttpRequestMessage(HttpMethod.Get, address + "items/1");
                msgHeaderV2.Headers.Add("ApiVersion", "2");
                var resHeaderV2 = client.SendAsync(msgHeaderV2).Result;
                Print(resHeaderV2);
